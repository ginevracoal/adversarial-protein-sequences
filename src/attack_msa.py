...
if args.load:
	...
	for seq_idx, single_sequence_data in tqdm(enumerate(data), total=len(data)):
		...
		dist_df = atk.build_distances_df(name=name, original_sequence=original_sequence, original_batch_tokens=batch_tokens,
			target_token_idxs=target_token_idxs, msa=msa, first_embedding=first_embedding, 
...
plot_cmap_distances(cmap_df, R=20, keys=perturbations_keys, distances_df=distances_df, filepath=out_plots_path, filename=out_filename)
